...
const Header = () => {
  ...
  return (
    <header className="header">
      ...
      <div className="top">
        <div className="logo">
          <img loading="lazy" src="/assets/Ina/logo.png" height={40} width={"auto"} style={{ objectFit: "contain" }} alt="Logo" />
          {/* <h3 className="text-16 text-700">CX Agent</h3> */}
        </div>

        <div className="right">
          <div className="lg-link-item">
            {menuItems.map((link) => <Link key={link.title} to={link.url}>{link.title}</Link>)}
          </div>
          <SearchSvg />
          <button
...
const SearchSvg = () => {
  return <svg width="25" height="25" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
